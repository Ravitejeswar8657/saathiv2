...
| `pa_schedule.html` | PA schedule upload. "Paste MP's schedule message" takes the raw daily broadcast (Telugu/English) and AI-splits it into one event per engagement, reviewed and edited in a preview before "Save all" commits them in one batch; the manual "Add new event" form is unchanged beneath it. Event cards are clickable and open the post-event coverage modal |
...
| POST | `/api/schedule/parse-text` | Split a pasted schedule broadcast into one preview event per engagement (JSON `{text}`) — extraction only, nothing is written |
| POST | `/api/schedule` | Add event; auto-finds nearby contacts by mandal using Fuse.js. Takes either a single-event body or a batch `{items:[…]}` (what the parse-text review flow saves), both through `scheduleOneEvent` |
...
The schedule broadcast has one extractor of its own, and it is the odd one out:

- `extractScheduleFromText(text, eventTypes, mandals, todayIST)` — splits an MP's daily schedule message into one object per engagement (`event_name`/`date`/`time`/`address`/`village`/`mandal`/`description`/`event_type`). Every other extractor in the file returns a single OBJECT because its source is one grievance or one report; this one wraps its object schema in a top-level ARRAY, because one message is many events. `callGemini` needed no change for that — it forwards whatever `responseSchema` it is handed. The prompt carries the Telugu time-of-day vocabulary (ఉదయం/మధ్యాహ్నం/సాయంత్రం/రాత్రి) needed to resolve a bare hour to an unambiguous 24-hour time, and is told to reuse the message's header date for every bullet that doesn't state one of its own. `mandal` is matched against `listMandals()` and left empty rather than guessed, because a district-level venue like a Collectorate is not a mandal. `event_type` is enum-constrained to `SCHEDULE_EVENT_TYPES` in `server/index.js` — the same 7 values as the page's `#sch-event-type` dropdown, and deliberately not a shared taxonomy module: `events.event_type` has no `CHECK` constraint to keep in step, unlike `REPORT_TAXONOMY`.
...
Errors thrown are `GeminiError` (carrying `status`/`retriable`) with a message written for the PA reading it; the provider's raw JSON body goes to `console.error` only. That matters because `mergeGrievanceExtraction`/`mergeReportExtraction` splice a failed source's message straight into the record's description. When *every* source of one submission fails, the route returns 503 (429 for a rate limit, 500 for a key or model id Google rejects outright) via `extractionFailureStatus` in `server/index.js` and the page toasts it, instead of handing staff a preview whose description is nothing but `(extraction failed — …)`. All three AI intakes go through that helper — `/api/grievances/log`, `/api/campaign-reports/log` and `/api/schedule/parse-text` — plus the bulk `/api/grievances/upload`, where every form failing at once means a service outage rather than 20 unreadable photos.
